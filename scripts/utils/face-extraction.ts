...
export async function extractFaceCrops(
  ...
  for (const filename of sidecars) {
    ...
    const sidecar = (await fs.readJSON(join(sam3Dir, filename))) as SAM3Sidecar;
    if (sidecar.segmentation_predictions.length === 0) continue;
    ...
    console.log(
      `   ✓ page-${padded}: ${faces.length} face/head detections → ${crops.filter((c) => c.pageNumber === pageNum).length} crops`,
    );
  }

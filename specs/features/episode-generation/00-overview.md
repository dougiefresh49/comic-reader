...
| Shot type | Model | Reason |
|-----------|-------|--------|
| Any shot with character faces | `kling-3.0` | No provenance requirement, accepts any image input |
| Establishing / atmosphere / no faces | `seedance-2.0` | Best motion quality for landscape/environment shots |
| Character reference images | `seedream-v5-lite` | Style anchor for Kling input conditioning |
| Scene panels (dialogue/action) | `seedream-v5-lite` | Reference-aware image generation |
| Optional background music | `stable-audio-3` | Venice ambient audio generation |

**Note on Seedance face restriction:** Seedance 2.0 requires input images generated by `seedream-v5-lite`. All character-face shots use Kling 3.0 to avoid this constraint entirely. Only face-free shots (establishing, atmosphere, title cards) use Seedance.

...
const BUBBLE_TYPES = ["SPEECH", "NARRATION", "CAPTION", "SFX", "BACKGROUND"] as const;
...
function BubbleDetail({
  ...
  const readingIndex = bubble.box_2d.index !== undefined ? bubble.box_2d.index + 1 : "?";
  ...
  return (
    <div className="flex flex-col gap-3 p-3">
      ...
      {/* textWithCues */}
      <label className="flex flex-col gap-1">
        <span className="text-xs font-medium text-neutral-400">textWithCues</span>
        <textarea
          value={bubble.textWithCues ?? ""}
          onChange={(e) => onChange({ textWithCues: e.target.value || undefined })}
          rows={3}
          className="w-full resize-none rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-sm text-neutral-100 focus:border-cyan-500 focus:outline-none"
        />
      </label>
      ...
      {bubble.aiReasoning && (
        <div className="flex flex-col gap-1">
          ...
          {aiExpanded && (
            <div className="max-h-32 overflow-y-auto rounded border border-neutral-700 bg-neutral-900/50 px-2 py-1.5 text-xs text-neutral-400">
              {bubble.aiReasoning}
            </div>
          )}
        </div>
      ...
      {/* Bounding Box */}
      <div className="flex flex-col gap-1">
        <span className="text-xs font-medium text-neutral-400">Bounding Box (%)</span>
      ...
      <div className="flex flex-col border-t border-neutral-800">
        ...
        <div className="max-h-48 overflow-y-auto">
          {bubbles
            ...
            .map((b) => {
              const isRedo = redoSet.has(b.id);
              const isSelected = b.id === selectedId;
              const readingIndex = b.box_2d.index !== undefined ? b.box_2d.index + 1 : "?";
              const previewText =
                b.ocr_text.length > 35
                  ? b.ocr_text.slice(0, 35) + "…"
                  : b.ocr_text;
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-baseline gap-2 px-3 py-1.5 text-left text-xs hover:bg-neutral-800 ${
                ...
                >
                  ...
                  <span className="w-20 shrink-0 truncate font-medium text-neutral-300">
                    {b.speaker ?? "[unassigned]"}
                  </span>
                  <span className="flex-1 truncate text-neutral-500">
                    &ldquo;{previewText}&rdquo;
                  </span>
                  <span className={`shrink-0 text-[10px] ${statusColor(b, isRedo)}`}>
                    {statusLabel(b, isRedo)}
                  </span>
                </button>
              );
            })}
